...
class StatelessTokenGeneratorTestCase(unittest.TestCase):
    ...
    def test_create_access_token_data_no_expiration(self):
        ...
        self.assertEqual(len(result["access_token"]), self.token_len_with_grant_type)
    ...
    def test_check_access_token_for_diff_secret_key(self):
        ...
        result1 = generator.create_access_token_data(data=None, scopes=None, grant_type='test_grant_type',
                                                    user_id=None, client_id=None)

        self.assertEqual(len(result["access_token"]), self.token_len_with_grant_type)
        self.assertEqual(result["token_type"], "Bearer")

        self.assertEqual(len(result1["access_token"]), self.token_len_with_grant_type)
        self.assertEqual(result1["token_type"], "Bearer")

        self.assertEqual(result["access_token"], result1["access_token"])
    ...
    def test_create_access_token_data_with_expiration_scopes(self):
        generator = StatelessTokenGenerator(self.sekret_key)
