...
class ClientNotFoundError(Exception):
    """
    Error raised by an implementation of :class:`oauth2.store.ClientStore` if a client does not exists.
    """
    pass
...
class InvalidSiteAdapter(Exception):
    """
    Raised by :class:`oauth2.grant.SiteAdapterMixin` in case an invalid site adapter was passed to the instance.
    """
    pass
...
class OAuthBaseError(Exception):
    ...
    :param error_uri: Set this to delivery an URL to your documentation that describes the error. (optional)
...
class OAuthInvalidNoRedirectError(OAuthInvalidError):
    """
    Indicates an error during validation of a request.
    The provider will not inform the client about the error by redirecting to it.
    This behaviour is required by the Authorization Request step of the Authorization Code Grant and Implicit Grant.
    """
    pass
...
class UserNotAuthenticated(Exception):
    """
    Raised by a :class:`oauth2.web.SiteAdapter` if a user could not be authenticated.
    """
    pass
